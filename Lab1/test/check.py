...
def write_address():
    file = open("acessos.log", "w")
    offset = 3
    index = 4
    tag = 24 - offset - index
    for lineIndex in range(numberOfLines):
        lines[lineIndex] = lines[lineIndex][9:9+tag] + " " + lines[lineIndex][9+tag:9+tag+index] + " " + lines[lineIndex][9+tag+index:9+tag+index+offset] + "\t" + lines[lineIndex][9+tag+index+offset:]
